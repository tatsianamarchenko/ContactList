...
class ContactsViewController: UIViewController {
	...
	private let contactStorageManager = ContactStorageManager()
	...
	override func viewDidLoad() {
		...
		switch contactStorageManager.authorizationStatus {
		case .notDetermined :
			addAccessButton()
		...
		case .authorized:
			...
			let cached = contactStorageManager.fetchCachedContacts()
		...
		if longPressGestureRecognizer.state == UIGestureRecognizer.State.began {
			...
			if let indexPath = contactsViewTable.indexPathForRow(at: touchPoint) {
				let alert = UIAlertController(
					title: "\(ContactStorageManager.contactsSourceArray.contacts[indexPath.row].name)",
					message: "\(ContactStorageManager.contactsSourceArray.contacts[indexPath.row].phoneNumber)",
					preferredStyle: .actionSheet)
				
				alert.addAction(UIAlertAction(title: NSLocalizedString("copy", comment: ""), style: .default, handler: { _ in
					UIPasteboard.general.string = ContactStorageManager.contactsSourceArray.contacts[indexPath.row].phoneNumber
				}))
				alert.addAction(UIAlertAction(title: NSLocalizedString("share", comment: ""),
											  style: .default,
											  handler: { [self] _ in
					let number = ContactStorageManager.contactsSourceArray.contacts[indexPath.row].phoneNumber
				...
				alert.addAction(UIAlertAction(title: NSLocalizedString("delete", comment: ""),
											  style: .default,
											  handler: { [self] _ in
					ContactStorageManager.contactsSourceArray.contacts.remove(at: indexPath.row)
					contactStorageManager.saveToDisk()
					contactsViewTable.beginUpdates()
					contactsViewTable.deleteRows(at: [indexPath], with: .fade)
					contactsViewTable.endUpdates()
					if ContactStorageManager.contactsSourceArray.contacts.isEmpty {
						addAccessButton()
					}
				}))
				alert.addAction(UIAlertAction(title: NSLocalizedString("cancel", comment: ""), style: .cancel, handler: nil))
	...
	@objc func accessToContacts () {
		contactStorageManager.authorizationStatus = CNContactStore.authorizationStatus(for: CNEntityType.contacts)
		switch contactStorageManager.authorizationStatus {
		case .authorized : loadContacts()
		case .notDetermined:
			contactStorageManager.contactStore.requestAccess(for: .contacts) { [self] access, error in
				if access {
					loadContacts()
				} else {
	...
	private func loadContacts() {
		DispatchQueue.global(qos: .userInteractive).async { [self] in
			...
			contactStorageManager.loadContacts()
...
extension ContactsViewController: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		return ContactStorageManager.contactsSourceArray.contacts.count
	}
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		if let cell = contactsViewTable.dequeueReusableCell(withIdentifier: ContactCell.cellIdentifier, for: indexPath)
			as? ContactCell {
			let contact = ContactStorageManager.contactsSourceArray.contacts[indexPath.row]
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		let model = ContactStorageManager.contactsSourceArray.contacts[indexPath.row]
